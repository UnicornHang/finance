...
**定位**：用户上传的图片和文件是聊天附件，不是工具调用记录，也不只服务于发票。
...
|---|---|
| MinIO | 文件字节 |
| `chat_files` | 附件元数据、所属会话、绑定的消息、识别状态 |
| `invoices` / `contracts` | 确认后的业务单据。`chat_files.invoice_id` / `contract_id` 指向它们 |
| `messages.tool_calls` | 预留给模型工具轨迹，不再存附件 |
| `messages.attachments` | 迁移过渡列。列表接口优先读 `chat_files` |

上传中、上传失败只存在于输入框。表里有行即表示已经上传成功。

### 23.2 状态

`recognize_status`：`pending`（已上传未发送或刚发送）→ `running` → `succeeded` 或 `failed`。

`intent`：`invoice` / `contract` / `chat`。普通图片和文件走 `chat`，处理完同样记成功或失败。

`file_kind`：`image` 或 `file`。

### 23.3 模型上下文

组装历史时，按 `message_id` 取出附件，把文件名、意图、识别状态附在该条用户消息后面，再送给模型。

### 23.4 同日修复

- 拉历史时按文件哈希合并，避免刚发出的附件被旧响应清掉。
- 流式进度句与落库正文视为同一轮，避免同一段回复显示两次。
