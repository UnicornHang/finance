        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {configs.map((cfg) => (
            <ConfigCard
              key={cfg.id}
              cfg={cfg}
              editing={editing === cfg.scene}
              onEdit={() => setEditing(cfg.scene)}
...
function ConfigCard({ cfg, editing, onEdit, onCancel }: CardProps) {
        ...
        {editing ? (
          <div className="space-y-3">
            <FieldInline label="Provider">
              <select
                className="flex h-10 w-full rounded-md border border-line bg-surface px-3 py-2 text-body-md focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <FieldInline label="Max Tokens">
                <Input
                  ...
                  onChange={(e) =>
                    setState((s) => ({ ...s, max_tokens: Number(e.target.value) }))
                  }
                />
              </FieldInline>
            </div>
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-line-subtle">
              <Button variant="secondary" size="sm" onClick={onCancel} disabled={save.isPending}>
                取消
              </Button>
              <Button
                size="sm"
